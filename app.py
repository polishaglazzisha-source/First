...
try:
    ...
    SERVICE_INFO, SHEET_ID = None, None

st.sidebar.write('gcp_ok:', SERVICE_INFO is not None)
st.sidebar.write('sheet_id_ok:', bool(SHEET_ID))

if SERVICE_INFO is None or not SHEET_ID:
    st.error('Нет конфигурации Google Sheets. Проверь secrets.')
    st.stop()
...
q6_items = [
    ...
    'Свой вариант'  # <— для него будет чекбокс, а не ползунок
]
...
HEADERS = [
    ...
    *[f'q6_{name}' for name in q6_items],     # для «Свой вариант» будет 'Включено'/''
    'q6_free_text',
    *[f'q6_{name}_any' for name in q6_items], # для «Свой вариант» any=1 при чекбоксе
...
st.caption('Для пунктов ниже укажите частоту. Для «Свой вариант» — просто отметьте чекбокс и опишите.')
q5 = {}
for item in q5_items[:-1]:
    q5[item] = st.radio(item, FREQ3, horizontal=True, index=2, key=f'q5_{item}')
q5_own_enabled = st.checkbox('П.5 — добавить «Свой вариант»')
...
# «Свой вариант» — чекбокс + текст (без ползунка)
q6_own_enabled = st.checkbox('П.6 — добавить «Свой вариант»')
q6['Свой вариант'] = 'Включено' if q6_own_enabled else ''
q6_free_text = ''
if q6_own_enabled:
...
locality = st.selectbox('Место жительства', [
    ...
    'Большой город (100–250 тыс.)', 'Средний город (50–100 тыс.)', 'Малый город и/или посёлок (<50 тыс.)'
])
...
if submitted:
    ...
    # П.4 — если «Свой вариант» выбран, текст обязателен (кириллица)
    if 'Свой вариант' in q4_vals:
        if not q4_free or not _has_cyrillic(q4_free):
            errors.append('П.4: «Свой вариант» — обязательно, введите текст кириллицей.')

    # П.5 — если «Свой вариант» включён, текст обязателен (кириллица)
    ...
    # п.6 — значения (числа) + индикаторы; для «Свой вариант» — флаг
    for name in q6_items[:-1]:
        v = q6.get(name, 0)
        row[f'q6_{name}'] = v
        row[f'q6_{name}_any'] = 1 if v > 0 else 0
    row['q6_Свой вариант'] = q6.get('Свой вариант', '')
    row['q6_free_text'] = q6_free_text
    row['q6_Свой вариант_any'] = 1 if q6.get('Свой вариант') else 0
