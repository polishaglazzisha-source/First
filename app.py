...
Original file is located at
    https://colab.research.google.com/drive/1JYFn333wkSz9JX9i3CO0in4Ux6SaLkSS
"""
...
if SERVICE_INFO is None and os.getenv('GOOGLE_SERVICE_ACCOUNT_JSON'):
    import json as _pyjson
    SERVICE_INFO = _pyjson.loads(os.environ['GOOGLE_SERVICE_ACCOUNT_JSON'])
if SHEET_ID is None:
    SHEET_ID = os.getenv('GOOGLE_SHEET_ID')

if SERVICE_INFO is None or SHEET_ID is None:
    st.error('Нет конфигурации Google Sheets. Добавьте secrets: gcp_service_account и google_sheet_id.')
    st.stop()
...
q6_items = [
    ...
    'Жертвование денег на реализацию коммерческих проектов (запись музыкальных альбомов, съемка фильмов и т.д.)',
...
# --------------------
# Фиксированный порядок колонок (шапка)
# --------------------
HEADERS = [
...
with st.form('survey', clear_on_submit=False):
    ...
    st.caption('Нажимая «Отправить», вы подтверждаете, что ознакомлены с целями исследования. Данные хранятся в обобщённом виде.')
