...
st.markdown(
    'Здравствуйте! ... (интро как раньше, сокращено для примера)'
)
...
# --------------------
# ВИДЖЕТЫ (без st.form, чтобы зависимости появлялись сразу)
# --------------------
...
edu = st.selectbox('Образование', [
    'Основное общее (9 классов)', 'Среднее общее (11 классов)',
    'Среднее профессиональное (колледж/техникум)',
    'Высшее I степени (бакалавриат)', 'Высшее II степени (специалитет/магистратура)',
    'Высшее с учёной степенью'
])
sector = st.selectbox('Сфера занятости', [
    'Рабочий', 'Инженерно-технические работники на предприятии',
    'Специалист в непроизводственной сфере (учитель, врач, работник науки, культуры и т.п.)',
    'Работник управленческого аппарата', 'Военнослужащий', 'Сотрудник органов внутренних дел',
    'Предприниматель', 'Пенсионер', 'Учащийся, студент', 'Домохозяйка', 'Не работаю', 'Другое'
])
...
if submitted:
    ...
    if q5.get('Свой вариант') == 'Включено':
        if not q5_free_text or not _has_cyrillic(q5_free_text):
            _err('П.5: «Свой вариант» — обязательно, введите текст кириллицей.')

    # П.6 — если «Свой вариант» ≠ 0, текст обязателен и кириллицей
    if q6.get('Свой вариант', 0) != 0:
        if not q6_free_text or not _has_cyrillic(q6_free_text):
            _err('П.6: «Свой вариант» — обязательно, введите текст кириллицей.')

    # 7–10 — если ДА, все три обязателены и кириллицей
    if q7 == 'ДА':
        if not q8 or not _has_cyrillic(q8):
            _err('П.8: обязательный ответ (кириллица).')
        if not q9 or not _has_cyrилlic(q9):
            _err('П.9: обязательный ответ (кириллица).')
        if not q10 or not _has_cyrillic(q10):
            _err('П.10: обязательный ответ (кириллица).')

    if not consent:
        _err('Нужно согласие на участие.')
