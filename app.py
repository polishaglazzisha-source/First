...
with st.form('survey', clear_on_submit=False):
    ...
    st.markdown('---')
    st.subheader('4. 4. Завершите предложение, пожалуйста, продолжив его выбором из нижеследующего перечня, что считаете для себя наиболее важным: «Оказавшись в трудной ситуации, я …')
    ...
    if q6.get('Свой вариант', 0) != 0:
        q6_free_text = st.text_input('Поясните «Свой вариант»')

    st.markdown('---')
    st.subheader('7. Есть ли у вас мечты, которые вы бы хотели реализовать?')
    q7 = st.radio(['ДА', 'НЕТ'], horizontal=True)
